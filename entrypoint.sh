...
if [ ! -d "./Migrations" ] || [ -z "$(ls -A ./Migrations 2>/dev/null)" ]; then
    ...
    dotnet ef migrations add InitialCreate --output-dir Migrations --project /app/src/DnDAPI.csproj || echo "Не удалось создать миграцию"
else
    echo "Миграции найдены, пропускаем создание."
fi

echo "Применяем миграции к базе..."
dotnet ef database update --project /app/src/DnDAPI.csproj || echo "Не удалось применить миграции (возможно, БД ещё не готова)."
